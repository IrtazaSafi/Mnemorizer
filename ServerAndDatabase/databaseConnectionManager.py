...
class databaseConnectionManager:
    def __init__(self):
        ...
        self.queryExecutor.execute("SET GLOBAL connect_timeout=28800")
        self.queryExecutor.execute("SET SESSION query_cache_type=0")
    ...
    def fetchMnemonicsForWord(self, _id):
        #self.queryExecutor.execute("SELECT SQL_NO_CACHE * FROM words,mnemonics")
        self.queryExecutor.execute(
            "SELECT DISTINCT SQL_NO_CACHE mnemonics.id, mnemonics.mnemonic,mnemonics.wordid,mnemonics.creatorid,mnemonics.latitude,mnemonics.longitude,mnemonics.score  FROM words,mnemonics WHERE mnemonics.wordid = %s",
            (_id,))
        self.db.commit()
        return self.queryExecutor.fetchall()
    ...
    def createMnemonic(self, mnemonic, wordid, creatorid, latitude, longitude):
        try:
            ...
            self.queryExecutor.execute(
                "INSERT INTO mnemonics(id,mnemonic,wordid,creatorid,latitude,longitude,score) VALUES (%s,%s,%s,%s,%s,%s,%s)",
    ...
    def viewMnemonicForWord(self,wordid):
        self.queryExecutor.execute("SELECT  mnemonic FROM mnemonics WHERE wordid =%s",(wordid))
        for row in self.queryExecutor.fetchall():
            print(row[0])
...
#dbConnection.viewMnemonicForWord(2)
